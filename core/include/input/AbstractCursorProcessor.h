#ifndef _AbstractCursorProcessor_h_
#define _AbstractCursorProcessor_h_
#include "utils/include/prerequisites.h"
#include "preinput.h"

#include "AbstractComponentProcessor.h"
//#include "InputCursor.h"
//#include "InputEvent.h"
//#include "AbstractCursorInputEvt.h"
//#include "scenegraph/GameObject.h"

typedef std::list<InputCursor*> InputCursorList;

class AbstractCursorProcessor:public AbstractComponentProcessor
{
protected:
	
    InputCursorList activeCursors;
    InputCursorList freeCursors;
...
//	private HashMap<InputCursor, AbstractCursorProcessor> cursorToLockLostInputProcessor = new HashMap<InputCursor, AbstractCursorProcessor>();
    typedef HashMap<InputCursor*, AbstractCursorProcessor*> CursorToCursorProcessorMap;
    CursorToCursorProcessorMap cursorToLockLostInputProcessor;
...
public:
	...
	AbstractCursorProcessor(bool stopPropagation):AbstractComponentProcessor(stopPropagation){
		lockPriority = 1.0f;
	}
	void init(bool stopPropagation)
	{
        AbstractComponentProcessor::init(stopPropagation);
	}

	bool isInterestedIn(InputEvent &inputEvt);


	void preProcessImpl(InputEvent &inputEvent);
...
//	}
    InputCursorList& getCurrentComponentCursors(){
...
//	}
    InputCursorList& getCurrentComponentCursorsArray(){
//		return this.activeCursors.toArray(new InputCursor[getCurrentComponentCursors().size()]);
        return activeCursors;
	}
...
//	}
    ...
    InputCursorList& getFreeComponentCursorsArray(){
//        List<InputCursor> freeCursors = getFreeComponentCursors();
//        return freeCursors.toArray(new InputCursor[freeCursors.size()]); 
        return getFreeComponentCursors();
    }
...
//		List<InputCursor> activeCursorsOnComp = getCurrentComponentCursors();
...
//		return freeCursors;
//	}
    InputCursorList& getFreeComponentCursors();
...
//	}
	 ...
	 */
	InputCursor* getFarthestFreeComponentCursorTo(InputCursor *cursor){
		return getFarthestFreeCursorTo(cursor);
	}

    InputCursor* getFarthestFreeCursorTo(InputCursor *cursor);
	/**
	 * Returns the most far away component cursor to the specified cursor.
	 * Only returns a cursor if it is free to use and could be locked by this processor.
	 * 
	 ...
	 */
	InputCursor* getFarthestFreeCursorTo(InputCursor *cursor, InputCursorList& excludedFromSearch);
...
//	}
	 ...
	 */
	bool isCursorDistanceGreater(InputCursor *reference, InputCursor *oldCursor, InputCursor *newCursor);
...
//	}
	 ...
	 */
	void setLockPriority(float gesturePriority) {
		lockPriority = gesturePriority;
	}
...
protected:
	 ...
	 */
#if 0
	bool canLock(InputCursor *cursors, int count){
		int locked = 0;
...
#endif
    bool canLock(InputCursor *cursors);
	bool canLock(InputCursorList& cursors);
...
//		return locked == cursors.length;
//	}
    bool getLock(InputCursor *cursors);
    bool getLock(InputCursorList cursors);
	 ...
	 * @param cursors the cursors
	 */
	void unLock(InputCursor *cursors);
...
//	}	
	...
	void cursorFreed(InputCursor *cursor);

	void cursorLostLock(InputCursor *cursor, AbstractCursorProcessor *lockinProcessor);
	 ...
	 */
	virtual void cursorLocked(InputCursor *cursor, IInputProcessor *lockingprocessor) = 0;
	 ...
	 * @param cursor the cursor
	 */
	virtual void cursorUnlocked(InputCursor *cursor) = 0;
	 ...
	 * @param currentEvent
	 */
	virtual void cursorStarted(InputCursor *inputCursor, AbstractCursorInputEvt &currentEvent) = 0;
	 ...
	 * @param currentEvent
	 */
	virtual void cursorUpdated(InputCursor *inputCursor, AbstractCursorInputEvt &currentEvent) = 0;
	 ...
	 * @param currentEvent
	 */
	virtual void cursorEnded(InputCursor *inputCursor, AbstractCursorInputEvt &currentEvent) = 0;
	

};


#endif /* _AbstractCursorProcessor_h_ */
