...
#include "asset/ResourceGroupManager.h"
...
class ParticleSystemManager;
    ...
    @par
		...
		SceneNode. 
    */
    class ParticleSystem : public StringInterface, public IMesh
    {
    public:
        ...
        /** Command object for quota (see ParamCommand).*/
        class CmdMinQuota : public ParamCommand
        {
        public:
            String doGet(const void* target) const;
            void doSet(void* target, const String& val);
        ...
        /** Command object for quota (see ParamCommand).*/
        class CmdMaxQuota : public ParamCommand
        {
        public:
            String doGet(const void* target) const;
            void doSet(void* target, const String& val);
		...
		/** Command object for always generate (see ParamCommand).*/
		class CmdAlwaysGenerate : public ParamCommand
		{
		public:
			String doGet(const void* target) const;
			void doSet(void* target, const String& val);
		...
		/** Command object for always generate (see ParamCommand).*/
		class CmdEnableTouchEvent : public ParamCommand
		{
		public:
			String doGet(const void* target) const;
			void doSet(void* target, const String& val);
		};
		class CmdIsEmitting : public ParamCommand
		{
		public:
			String doGet(const void* target) const;
			void doSet(void* target, const String& val);
        ...
        class  CmdMinEmitDelayTime : public ParamCommand
        {
        public:
            String doGet(const void* target) const;
            void doSet(void* target, const String& val);
        ...
        /// Command object for particle emitter  - see ParamCommand
        class  CmdMaxEmitDelayTime : public ParamCommand
        {
        public:
            String doGet(const void* target) const;
            void doSet(void* target, const String& val);
        };
        class  CmdDefaultDimensionsFactor : public ParamCommand
        {
        public:
            String doGet(const void* target) const;
            void doSet(void* target, const String& val);
        };
        class  CmdDefaultWidthFactor : public ParamCommand
        {
        public:
            String doGet(const void* target) const;
            void doSet(void* target, const String& val);
        };
        class  CmdDefaultHeightFactor : public ParamCommand
        {
        public:
            String doGet(const void* target) const;
            void doSet(void* target, const String& val);
        };
        class  CmdAspectratio : public ParamCommand
        {
        public:
            String doGet(const void* target) const;
            void doSet(void* target, const String& val);
		...
		class CmdEnableDynamicBinding : public ParamCommand
		{
		public:
			String doGet(const void* target) const;
			void doSet(void* target, const String& val);
		...
		GameObject* getParentGameObject() { return mParentGo; }
		void setParentGameObject(GameObject* parentGo) { mParentGo = parentGo; }
//		ParticleSystemListener* mParticleSystemListener;

//        void setParticleSystemListener(ParticleSystemListener* listener) { mParticleSystemListener = listener; };
//        ParticleSystemListener* getParticleSystemListener() { return mParticleSystemListener; };

        /// Default constructor required for STL creation in manager
        ParticleSystem(Root* root, ParticleSystemManager* pParticleSystemManager);
        /** Creates a particle system with no emitters or affectors.
        @remarks
            You should use the ParticleSystemManager to create particle systems rather than creating
            them directly.
        */
        ParticleSystem(Root* root, ParticleSystemManager* pParticleSystemManager, const String& name, const String& resourceGroup);
        ...
        */
        ...
        size_t getMinParticleQuota() const;
        void setMinParticleQuota(size_t minSize);
        size_t getMaxParticleQuota() const;
        void setMaxParticleQuota(size_t maxSize);
        void setParticleQuota(size_t minSize, size_t maxSize);
        size_t genParticleQuota();
...
//        void setBounds(const AxisAlignedBox& aabb);
		...
		*/
		...
		void setStacks(uint16 stacks);
		void setSlices(uint16 slices);
		unsigned short getStacks() const;
		unsigned short getSlices() const;

		bool getAlwaysGeneratePartices(void) const { return mIsAlwaysGeneratePartices; }
		void setAlwaysGeneratePartices(bool enabled) { mIsAlwaysGeneratePartices = enabled; }

		bool hasTouchEvent(void) const { return mHasTouchEvent; }
		void enableTouchEvent(bool enabled) { mHasTouchEvent = enabled; }

		bool isDynamicBinding(void) const { return mDynamicBinding; }
		void enableDynamicBinding(bool dynamicBinding) { mDynamicBinding = dynamicBinding; }

		void onStop(bool beStop);
		void resize(Real width, Real height);

		ParticleSystem* copyParametersFrom(const ParticleSystem& rhs);
		unsigned int getEmitterNumber() { return mEmitters.size(); }
...
protected:
        ...
        static CmdMinQuota msMinQuotaCmd;
        static CmdMaxQuota msMaxQuotaCmd;
		...
		static CmdAlwaysGenerate msCmdAlwaysGenerate;
		static CmdEnableTouchEvent msEnableTouchEventCmd;
		static CmdIsEmitting msIsEmittingCmd;
		static CmdMinEmitDelayTime msMinEmitDelayTimeCmd;
		static CmdMaxEmitDelayTime msMaxEmitDelayTimeCmd;
		static CmdDefaultDimensionsFactor msDefaultDimensionsFactorCmd;
		static CmdDefaultWidthFactor msDefaultWidthFactorCmd;
		static CmdDefaultHeightFactor msDefaultHeightFactorCmd;
		static CmdAspectratio msAspectratioCmd;
		static CmdEnableDynamicBinding msEnableDynamicBindingCmd;
...
//        AxisAlignedBox mAABB;
		...
		unsigned short mStacks;
		unsigned short mSlices;
...
//		Controller<Real>* mTimeController;
		...
		bool mIsAlwaysGeneratePartices;
		bool mHasTouchEvent;
		bool mDynamicBinding;

		// control relative default size for system
		bool mIsDefaultDimensionsFixed;
		Real mDefaultWidthFactor;
		Real mDefaultHeightFactor;
		Real mAspectratio;// height / width
		Real getDefaultWidthFactor() const { return mDefaultWidthFactor; }
		void setDefaultWidthFactor(Real defaultWidthFactor) {
			mDefaultWidthFactor = defaultWidthFactor;
			mIsDefaultDimensionsFixed = false;
		}
		Real getDefaultHeightFactor() const { return mDefaultHeightFactor; }
		void setDefaultHeightFactor(Real defaultHeightFactor) {
			mDefaultHeightFactor = defaultHeightFactor;
			mIsDefaultDimensionsFixed = false;
		}
		Real getDefaultDimensionsFactor() const { return mDefaultWidthFactor; }
		void setDefaultDimensionsFactor(Real defaultFactor) {
			mDefaultWidthFactor = mDefaultHeightFactor = defaultFactor;
			mIsDefaultDimensionsFixed = false;
		}
		Real getAspectratio() const { return mAspectratio; }
		void setAspectratio(Real aspectratio) {	mAspectratio = aspectratio;	}
		void ResizeDefaultDimensions(Real deviceWidth, Real deviceHeight);

		// control the time for emit delay
		bool mIsEmitDelaySet;
		Real mMinEmitDelayTime;
		Real mMaxEmitDelayTime;
		Real mEmitRemainTime;
		bool mIsEmitDelaying;
		Real getMinEmitDelayTime() const { return mMinEmitDelayTime; }
		void setMinEmitDelayTime(Real minDelaySec) {
			mMinEmitDelayTime = minDelaySec;
			mIsEmitDelaySet = true;
		};
		Real getMaxEmitDelayTime() const { return mMaxEmitDelayTime; }
		void setMaxEmitDelayTime(Real maxDelaySec) {
			mMaxEmitDelayTime = maxDelaySec;
			mIsEmitDelaySet = true;
		};
		void setEmitDelayTime(Real delaySec) { mMinEmitDelayTime = mMaxEmitDelayTime = delaySec; };
		void setEmitDelayTime(Real minDelaySec, Real maxDelaySec) {
			mMinEmitDelayTime = minDelaySec;
			mMaxEmitDelayTime = maxDelaySec;
		};
		Real getEmitDelayTime() const { return mMinEmitDelayTime; };
	    Real genEmitDelayTime() {
	        if (mMinEmitDelayTime != mMaxEmitDelayTime) {
	            return mMinEmitDelayTime + (Math::UnitRandom() * (mMaxEmitDelayTime - mMinEmitDelayTime));
	        } else {
	            return mMinEmitDelayTime;
	        }
	    }
		bool isEmitDelaySet() const { return mIsEmitDelaySet; }
		bool isEmitDelaying() const { return mIsEmitDelaying; }

        typedef list<Particle*> ActiveParticleList;
        typedef list<Particle*> FreeParticleList;
...
//		static RadixSort<ActiveParticleList, Particle*, float> mRadixSorter;
        ...
        */
		...
		typedef list<ParticleEmitter*> FreeEmittedEmitterList;
		typedef list<ParticleEmitter*> ActiveEmittedEmitterList;
		typedef vector<ParticleEmitter*> EmittedEmitterList;
		typedef map<String, FreeEmittedEmitterList> FreeEmittedEmitterMap;
		typedef map<String, EmittedEmitterList> EmittedEmitterPool;
        ...
        */
        ...
        size_t mPoolSize;
        size_t mCurrentValidSize;
        size_t mMinValidSize;
        size_t mMaxValidSize;
