...
struct GpuConstantDefinition
{
	...
	String toString() {
		OString os;
		os<<"constType "<<constType<<" physicalIndex "<<physicalIndex<<" logicalIndex "<<logicalIndex<<" elementSize "<<elementSize<<" arraySize "<<arraySize<<" variability "<<variability;
		return os.str();
	}
...
struct GpuNamedConstants
{
	...
	String toString() {
		OString os;
		os<<"\tfloatBufferSize "<<floatBufferSize<<" intBufferSize "<<intBufferSize<<"\n";
		GpuConstantDefinitionMap::iterator iter = map.begin();
		while(iter != map.end()){
			os<<"\tname ["<<iter->first + "] "<<iter->second.toString() + "\n";
			iter++;
		}
		return os.str();
	}
...
typedef map<size_t, GpuLogicalIndexUse> GpuLogicalIndexUseMap;
...
public:
	...
	*/
	...
	public:
		...
		String toString() {
			OString os;
			os<<"\tparamType "<<paramType<<" physicalIndex "<<physicalIndex<<" elementCount "<<elementCount<<" variability "<<variability<<"\n";
			return os.str();
		}
	...
	typedef vector<AutoConstantEntry> AutoConstantList;

	typedef vector<GpuSharedParametersUsage> GpuSharedParamUsageList;
...
public:
	...
	*/
	...
	void _writeRawConstant(size_t physicalIndex, ColourValue& colour, size_t count = 4);
