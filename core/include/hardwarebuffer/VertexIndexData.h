...
public:
	void setHardwareBufferManagerBase(HardwareBufferManagerBase* mgr) { mMgr = mgr; }
	/** Constructor.
	@note
	...
	*/
	VertexData(HardwareBufferManagerBase* mgr, VertexDeclaration* dcl, VertexBufferBinding* bind);
	~VertexData();
	...
	*/
	ushort allocateHardwareAnimationElements(Root* root, ushort count, bool animateNormals);
};
...
protected:
	...
	HardwareBufferManagerBase* mMgr;
public:
	IndexData(HardwareBufferManagerBase* mgr);
	~IndexData();
