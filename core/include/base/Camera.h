...
#include "utils/include/PlaneBoundedVolume.h"
#include "Frustum.h"
...
protected:
	...
	*/
	...
	mutable vector<Plane> mWindowClipPlanes;
	...
	*/
	...
	virtual vector<Vector4> getRayForwardIntersect(const Vector3& anchor, const Vector3 *dir, Real planeOffset) const;

public:
	/** Standard constructor.
	*/
	Camera(Root*root, const String& name, SceneManager* sm);

	/** Standard destructor.
	*/
	virtual ~Camera();
    void calcProjection_View_Matrix(Real left, Real right, Real top, Real bottom);
    void calcProjection_View_Matrix(float width, float height);

    void cal(float &x, float &y, float z, float & hw, float& hh);
	/// Add a listener to this camera
	virtual void addListener(Listener* l);
	/// Remove a listener to this camera
	virtual void removeListener(Listener* l);

	void setAutoScreenSize(bool autoScreenSize);
	const bool getAutoScreenSize() const;
...
//	friend std::ostream& operator<<(std::ostream& o, const Camera& c);
	...
	*/
	void setAutoTracking(bool enabled, GameObject* const target = 0,
	...
	*/
	...
	const vector<Plane>& getWindowPlanes(void) const;

	/** Overridden from MovableObject */
	Real getBoundingRadius(void) const;
	/** Get the auto tracking target for this camera, if any. */
	GameObject* getAutoTrackTarget(void) const { return mAutoTrackTarget; }
	...
	*/
	virtual void forwardIntersect(const Plane& worldPlane, vector<Vector4>* intersect3d) const;
