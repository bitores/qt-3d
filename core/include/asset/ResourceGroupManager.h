...
{
private:
	typedef HashMap<String, ScriptLoader*> ScriptLoaderMap;
	ScriptLoaderMap mScriptLoaderMap;

	Archive* mActiveArchive;
	Root* mRoot;
public:
	ResourceGroupManager(Root* root);
	~ResourceGroupManager();

	void registerAssetParser(String key, ScriptLoader* parser);
	void unregisterAssetParser(String key);

	ScriptLoader* getAssetParser(String key);

	void destroyAssetParsers();

	void parseScript(const String& filename, const String& groupName/* = ResourceGroupManager::AUTODETECT_RESOURCE_GROUP_NAME*/);
	DataStreamPtr openResource(const String& filename, const String& groupName);
	void setActiveArchive(Archive* arch);
	Archive* getActiveArchive();
public:
