...
public:
	...
	*/
	...
	Image(uchar* pData, size_t uWidth, size_t uHeight, size_t depth, PixelFormat eFormat, bool autoDelete = false);
	...
	*/
	...
	/** Loads an image file.
		...
		@note
			...
			Image object.
	*/
	Image & load(Root* root, const String& strFileName, const String& groupName );
	...
	*/
	Image & load(Root* root, DataStreamPtr& stream, const String& type = StringUtil::BLANK );
	...
	*/
	Image & loadTwoImagesAsRGBA(Root* root, const String& rgbFilename, const String& alphaFilename,
	...
	@param alphaType The type of the alpha image. Used to decide what decompression
		...
		a header to identify the data.
	*/
	Image & loadTwoImagesAsRGBA(Root* root, DataStreamPtr& rgbStream, DataStreamPtr& alphaStream, PixelFormat = PF_BYTE_RGBA,
...
//	TextureFormat type;											// Image Type (GL_RGB, GL_RGBA)
//
//	bool bFreeOnDestroy;	// Whether free imageData when destroy object
//public:
//	Image(bool freeOnDestroy = true);
//	~Image();
//
//	int loadImageData(void* data);
